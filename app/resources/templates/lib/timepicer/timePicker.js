                ...
                return {
                    ...
                    link: function (scope, elm, attrs) {
                        var tem = "<div class='pickerContainer datetimeactive'>" +
                            ...
                            "<div class='col ' ng-if='!options.hideTime' ><ion-scroll on-scroll='scrollingEvent(\"time\")' delegate-handle='timeScroll' scrollbar-y='false' class='timeContent'>" + "<ul>" + "<li ng-style='time.selected ? { color: \"#0081b8\",fontWeight: \"bold\", fontSize: \"1.2em\"}:{}' ng-click='selectEvent(\"time\",$index)' ng-repeat='time in timeList'>{{time.text+':00-'}}{{time.text+1+':00'}}</li>" + "</ul>" + "</ion-scroll></div>" +
                            "</div>"
                        ...
                        var options = {
                            ...
                            timeSpan: attrs.timespan && parseInt(attrs.timespan) || 60, //时间间隔 默认 15分钟一个间隔 15/30
                        ...
                        function initTime(options) {
                            ...
                            for (var i = 0; i < options.timeNum; i++) {
                                ...
                                for (var j = 0; j < 60 / (options.timeSpan); j++) {
                                    ...
                                    scope.timeList.push(createDateTimeLi(0, top, data, false, data2));
                                }
                            }
                        ...
                        function getDefaultSelectTime(options) {
                            ...
                            if (options.minuteSkip || ((options.DateTime.getMinutes() / options)) != 0) {
                                options.minuteSkip = options.minuteSkip || -60;
                        ...
                        function setSelectDateTimeShow() {
                            ...
                            var timeTxt2=parseInt(scope.selectDateTime.time.item.text)+1+':00';
                            var timeTxt = scope.selectDateTime.time.item.text+':00'+'-'+timeTxt2;
                        ...
                        scope.ok = function () {
                            ...
                            setSelectDateTimeShow();
                            hide();
                            scope.$parent.modal2.show();
                            scope.$parent.modal.show();
                            scope.$parent.changeTime(datetime);
